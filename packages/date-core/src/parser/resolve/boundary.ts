import { expandTwoDigitYear, firstWeekdayAfter, firstWeekdayBefore, firstWeekdayOnOrAfter } from "../primitives/date-math";
...
import { BackwardRelativeModifierSet, ForwardRelativeModifierSet, type CalendarListPeriod, type CalendarRangePeriod, type DayGroupPeriod } from "../types";
import type {
...
export function resolveBoundary(
  ...
  switch (boundary.kind) {
    case "anchor-until": {
      const end = resolveBoundaryEndpoint(boundary.end, anchorDate, Temporal, context, lookups);
      return end && comparePlainDate(anchorDate, end) <= 0 ? { kind: "range", start: anchorDate, end } : null;
    }
    case "atom":
      return resolveAtomBoundary(boundary.input, anchorDate, Temporal, context, lookups);
    case "boundary-side": {
      const date = resolveBoundaryAsEndpoint(boundary.boundary, boundary.side, anchorDate, Temporal, context, lookups);
      return date ? { kind: "single", date } : null;
    }
    case "duration-from-anchor": {
      const start = resolveBoundaryAsEndpoint(boundary.anchor, "start", anchorDate, Temporal, context, lookups);
      return start ? { kind: "range", start, end: start.add(toDuration(boundary.amount, boundary.unit)) } : null;
    }
    case "month-range":
      return resolveMonthRange(boundary.month, resolveYearReference(boundary.year, anchorDate.year), Temporal);
    case "named-month-range":
      return resolveMonthRange(boundary.month, anchorDate.year, Temporal);
    case "ordinal-calendar-unit":
      return resolveOrdinalCalendarUnitBoundary(boundary, anchorDate, Temporal, context, lookups);
    case "ordinal-calendar-unit-span":
      return resolveOrdinalCalendarUnitSpanBoundary(boundary, anchorDate, Temporal, context, lookups);
    case "ordinal-day-group-in-range":
      return resolveOrdinalDayGroupBoundary(boundary, anchorDate, Temporal, context, lookups);
    case "ordinal-weekday-in-range":
      return resolveOrdinalWeekdayBoundary(boundary, anchorDate, Temporal, context, lookups);
    case "quarter-range":
      return resolveQuarterRange(boundary.quarter, resolveYearReference(boundary.year, anchorDate.year), Temporal);
    case "range": {
      const start = resolveBoundaryEndpoint(boundary.start, anchorDate, Temporal, context, lookups);
      const end = resolveBoundaryEndpoint(boundary.end, anchorDate, Temporal, context, lookups);
      return start && end && comparePlainDate(start, end) <= 0 ? { kind: "range", start, end } : null;
    }
    case "relative":
      return resolveRelativeExpression(boundary.expression, anchorDate, Temporal, context, lookups);
    case "relative-month":
      return resolveMonthRange(boundary.month, resolveRelativeMonthYear(boundary.modifier, boundary.month, anchorDate), Temporal);
    case "relative-quarter-range":
      return resolveRelativeQuarterRange(boundary.modifier, anchorDate, Temporal);
    case "shifted-anchor":
      return resolveShiftedAnchorBoundary(boundary, anchorDate, Temporal, context, lookups);
    ...
    case "unique-weekday-in-range":
      return resolveUniqueWeekdayBoundary(boundary, anchorDate, Temporal, context, lookups);
    case "week-range":
      return resolveWeekRange(boundary.week, resolveYearReference(boundary.year, anchorDate.year), Temporal);
  }
}
...
function resolveBareDateAnchor(input: string, anchorDate: PlainDate): PlainDate | null {
  switch (input) {
    case "now":
    case "today":
      return anchorDate;
...
function resolveBoundaryEndpoint(
  ...
  if (endpoint.kind === "relation") {
    const value = resolveBoundary(endpoint.boundary, anchorDate, Temporal, context, lookups);
    const related = value ? applyRelations(value, endpoint.relation) : null;
    return related?.kind === "single" ? related.date : null;
  }

  return resolveBoundaryAsEndpoint(endpoint.boundary, endpoint.side, anchorDate, Temporal, context, lookups);
}
...
function resolveBoundaryAsEndpoint(
  ...
  const value = resolveBoundary(boundary, anchorDate, Temporal, context, lookups);
  ...
  return value?.kind === "multiple" ? (side === "start" ? (value.dates[0] ?? null) : (value.dates.at(-1) ?? null)) : null;
}

// Example: `resolveYearReference({ kind: "relative", value: "next" }, 2026)` returns `2027`.
function resolveYearReference(year: YearReferenceSlice, anchorYear: number): number {
  if (year.kind === "explicit") {
    return expandTwoDigitYear(year.year, anchorYear);
  }

  if (year.kind === "relative") {
    return year.value === "next" ? anchorYear + 1 : year.value === "last" || year.value === "previous" ? anchorYear - 1 : anchorYear;
  }
...
function resolveMonthRange(month: number, year: number, Temporal: TemporalApi): DateValue {
...
function resolveQuarterRange(quarter: number, year: number, Temporal: TemporalApi): DateValue | null {
  if (quarter < 1 || quarter > 4) {
    return null;
  }

  const start = Temporal.PlainDate.from({ year, month: (quarter - 1) * 3 + 1, day: 1 });
  return { kind: "range", start, end: endOfCalendarMonth(start.add({ months: 2 })) };
}

// Example: `resolveRelativeQuarterRange("next", anchor, Temporal)` returns the quarter after the anchor quarter.
function resolveRelativeQuarterRange(modifier: RelativeModifier, anchorDate: PlainDate, Temporal: TemporalApi): DateValue {
  const currentQuarter = Math.floor((anchorDate.month - 1) / 3) + 1;
  const quarterOffset = modifier === "this" ? 0 : isForwardModifier(modifier) ? 1 : -1;
...
function resolveWeekRange(week: number, year: number, Temporal: TemporalApi): DateValue | null {
  if (week < 1) {
    return null;
  }

  const weekOneStart = startOfCalendarWeek(Temporal.PlainDate.from({ year, month: 1, day: 4 }), 1);
...
function resolveRelativeExpression(
  ...
  if (expression.kind === "bare") {
    return { kind: "single", date: resolveBareDateAnchor(expression.value, anchorDate) ?? anchorDate };
  }

  if (expression.kind === "from-now") {
    return { kind: "single", date: anchorDate.add(toDuration(expression.amount, expression.unit)) };
  }

  if (expression.kind === "leading-trailing-days") {
    return resolveLeadingOrTrailingDays(expression, anchorDate, Temporal, context, lookups);
  }

  return resolveRelativeModifierExpression(expression.modifier, expression.target, anchorDate, context);
}
...
function resolveLeadingOrTrailingDays(
  expression: Extract<RelativeExpressionSlice, { kind: "leading-trailing-days" }>,
  anchorDate: PlainDate,
  Temporal: TemporalApi,
  context: ResolvedParseDateContext,
  lookups: DateVocabularyLookups,
): DateValue | null {
  const range = resolveBoundary(expression.range, anchorDate, Temporal, context, lookups);
  if (range?.kind !== "range") {
    return null;
  }
  ...
  if (expression.count === 1) {
    return expression.skipHolidays && context.holidays?.includes(date) ? { kind: "multiple", dates: [] } : { kind: "single", date };
  }

  const value =
    expression.edge === "first"
      ? { kind: "range" as const, start: range.start, end: range.start.add({ days: expression.count - 1 }) }
      : { kind: "range" as const, start: range.end.subtract({ days: expression.count - 1 }), end: range.end };

  return comparePlainDate(value.start, range.start) >= 0 && comparePlainDate(value.end, range.end) <= 0
...
function resolveRelativeModifierExpression(
  ...
  switch (target.kind) {
    case "calendar-unit":
      return resolveCalendarUnitRange(modifier, target.unit, anchorDate, context);
    case "counted-duration":
      return modifier === "this" ? null : resolveDurationRange(modifier, target.count, target.unit, anchorDate, context);
    case "counted-weekday":
      return modifier === "this"
        ? null
        : { kind: "multiple", dates: resolveCountedWeekdays(modifier, target.count, target.weekday, anchorDate) };
    case "day-group":
      return resolveDayGroupRange(modifier, target.group, anchorDate, context);
    case "weekday":
      return { kind: "single", date: resolveRelativeWeekday(modifier, target.weekday, anchorDate) };
  }
...
function resolveCalendarUnitRange(
  ...
  if (unit === "week") {
    return { kind: "range", start: startOfCalendarWeek(date, context.weekStartsOn), end: endOfCalendarWeek(date, context.weekStartsOn) };
  }

  if (unit === "month") {
    return { kind: "range", start: startOfCalendarMonth(date), end: endOfCalendarMonth(date) };
  }

  return { kind: "range", start: startOfCalendarYear(date), end: endOfCalendarYear(date) };
}
...
function resolveCountedWeekdays(modifier: RelativeModifier, count: number, weekday: number, anchorDate: PlainDate): PlainDate[] {
...
function resolveDayGroupRange(
  ...
  const weekStart = startOfCalendarWeek(anchorDate.add({ weeks: offset }), context.weekStartsOn);
...
function resolveOrdinalCalendarUnitSpanBoundary(
  ...
  const range = resolveBoundary(boundary.range, anchorDate, Temporal, context, lookups);
  if (range?.kind !== "range" || boundary.startOrdinal > boundary.endOrdinal) {
    return null;
  }

  const startRange = selectOrdinalCalendarUnitRange(range, boundary.unit, boundary.startOrdinal);
  const endRange = selectOrdinalCalendarUnitRange(range, boundary.unit, boundary.endOrdinal);
...
function resolveOrdinalCalendarUnitBoundary(
  ...
  const selectedRanges = boundary.ordinals
    .map((ordinal) => selectOrdinalCalendarUnitRange(range, boundary.unit, ordinal))
...
function selectOrdinalCalendarUnitRange(range: Extract<DateValue, { kind: "range" }>, unit: CalendarListPeriod, ordinal: number): DateValue | null {
...
function resolveShiftedAnchorBoundary(
  ...
  const source = resolveBoundary(boundary.anchor, anchorDate, Temporal, context, lookups);
  const range = resolveBoundary(boundary.range, anchorDate, Temporal, context, lookups);
...
function resolveOrdinalWeekdayBoundary(
  ...
  const date = firstWeekdayOnOrAfter(range.start, boundary.weekday).add({ weeks: boundary.ordinal - 1 });
...
function resolveOrdinalDayGroupBoundary(
  ...
  const dates = expandWeekdaysBetween(range.start, range.end, boundary.group === "weekend" ? [6, 7] : [1, 2, 3, 4, 5]);
  const groupStartIndex = (boundary.ordinal - 1) * datesPerGroup(boundary.group);
  const selectedDates = dates.slice(groupStartIndex, groupStartIndex + datesPerGroup(boundary.group));
  return selectedDates.length === datesPerGroup(boundary.group)
    ? { kind: "range", start: selectedDates[0] as PlainDate, end: selectedDates[selectedDates.length - 1] as PlainDate }
    : null;
}
...
function resolveShorthandRangeListBoundary(
  ...
  const ranges = boundary.ordinals.map((ordinal) =>
    boundary.unit === "week" ? resolveWeekRange(ordinal, year, Temporal) : resolveMonthRange(ordinal, year, Temporal),
  );
...
function resolveRelativeMonthYear(modifier: RelativeModifier, month: number, anchorDate: PlainDate): number {
  if (isForwardModifier(modifier)) {
...
function applyHolidayExclusion(value: Extract<DateValue, { kind: "range" }>, skipHolidays: boolean, context: ResolvedParseDateContext): DateValue {
  return skipHolidays
    ? { kind: "multiple", dates: expandDatesBetween(value.start, value.end).filter((date) => !context.holidays?.includes(date)) }
    : value;
}
...
function startOfCalendarWeek(date: PlainDate, weekStartsOn: WeekdayIndex): PlainDate {
...
function endOfCalendarWeek(date: PlainDate, weekStartsOn: WeekdayIndex): PlainDate {
